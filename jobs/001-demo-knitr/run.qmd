...
params:
    JOB_DIR: NULL
    JOB_ROW: 0 # use zero-indexing here!
    JOB_TESTING: true
---
...
### Retrieve default parameters for interactive sessions

While RStudio may insert the default parameters from the yaml header into the
current session, VS Code and Positron do not do that. The following code block
creates the params list manually by reading the defaults from the yaml header.

```{r}
#| label: get-default-params

if (!exists("params")) {
  # retrieve defaults from this document
  current_file <- rstudioapi::getActiveDocumentContext()$path
  file_txt <- readr::read_file(current_file)
  knitr_params <- knitr::knit_params(file_txt)

  # parse knitr parameters list
  params <- list()
  for (param in names(knitr_params)) {
    if (param == "JOB_DIR") {
      # special handling for JOB_DIR; default to the parent dir of this file,
...
### Guess current job dir if JOB_DIR: NULL (default)

It is annoying the have to update the default "JOB_DIR" parameter. The following code
block inserts the current jobdir, if params$JOB_DIR is NULL (which is the default),
so the default does not have to be updated manually.
